...
1. **Execute Project Phase 1's ordered experiment sequence** (see `PROJECT_PLAN.md`) — reproduce the masking process (done), resolve the 2015 anomaly as a hard gate (done — genuine El Nino-plausible regime, not an artifact), the blackout-degradation curve stratified by staleness × latitude × season × location-ACF × drought regime (done — `notebooks/02_forecastability.ipynb` §9-10; the pooled curve genuinely does hide different subpopulations as expected, with per-location ACF quartile the dominant factor by a wide margin (0.40 RMSE spread at k=9) ahead of latitude band (0.19), season (0.08), and onset drought regime (0.07) — directly reprioritizes §7-A/G below: ACF/historical-signature features first, region-aware features second), then last-known-state baseline, staleness×location-dynamics, covariate shift, and the real GRACE mission timeline.
